...
public class MyTeamFormAdapter extends RecyclerView.Adapter<MyTeamFormAdapter.ViewHolder> {
    private List<ScheduleTeamFormModel> arrayList;
    private Context context;
    private DisplayMetrics dm;

    public MyTeamFormAdapter(MyTeamFormationActivity myTeamFormationActivity, List<ScheduleTeamFormModel> arrayList) {
        this.context = myTeamFormationActivity;
        this.arrayList = arrayList;
        dm = new DisplayMetrics();
        myTeamFormationActivity.getWindowManager().getDefaultDisplay().getMetrics(dm);
    }
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        final ScheduleTeamFormModel m = arrayList.get(holder.getAdapterPosition());
        ...
        if(m.getPlayer_photo() != null && !m.getPlayer_photo().isEmpty()) {
            Glide.with(context).load(m.getPlayer_photo()).placeholder(R.drawable.profiledefaultimg).dontAnimate().into(holder.mSquadimage);
        } else {
            Glide.with(context).load(R.drawable.profiledefaultimg).dontAnimate().into(holder.mSquadimage);
        }
