...
public class FirstTeamSquadAdapter extends RecyclerView.Adapter<FirstTeamSquadAdapter.MatchSquadViewHolder>{
    private Context mContext;
    private List<MySquadInfo> mSquadInfos = new ArrayList<>();

    public FirstTeamSquadAdapter(Context context, List<MySquadInfo> squadInfos) {
        mContext = context;
    ...
    public void onBindViewHolder(MatchSquadViewHolder holder, int position) {
        MySquadInfo squadInfo = mSquadInfos.get(position);
        holder.mPlayerNameView.setText(squadInfo.getPlayerName());
        if (squadInfo.getPlayerImage() != null) {
            Glide.with(mContext).load(squadInfo.getPlayerImage()).placeholder(R.drawable.profiledefaultimg).dontAnimate().into(holder.mPlayerImageView);
        } else {
